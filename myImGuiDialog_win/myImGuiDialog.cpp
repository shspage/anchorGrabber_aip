...
namespace myImGuiDialog
{
	...
	int runModal(HWND parentHwnd, MyDialogParms* parms, std::function<void(void)> callbackFunc)
	{
		int dialogResult = 0;
		...
		show_preview = true;
#endif
		if (show_preview) callbackFunc();  // # 初回のプレビューを表示
		...
		while (!done) {
			...
			// # ダイアログの部品。流用する際の可変要素。コードはmacと共通。
			{
				...
				ImGui::Begin("settings", &is_open, flag);
				...
				sprintf(buf_drag, LABEL_TOLERANCE, (int)MIN_HIT_TOLE,
					(int)MAX_HIT_TOLE);
				if (ImGui::InputInt(buf_drag, &(parms->tolerance_drag))) {
					parms->tolerance_drag = fmax(MIN_HIT_TOLE, fmin(MAX_HIT_TOLE, parms->tolerance_drag));
				...
				if (ImGui::InputInt(buf_safe, &(parms->tolerance_safe_click))) {
					parms->tolerance_safe_click = fmax(MIN_SAFE_CLICK, fmin(MAX_SAFE_CLICK, parms->tolerance_safe_click));
					if (show_preview) callbackFunc();
				}
